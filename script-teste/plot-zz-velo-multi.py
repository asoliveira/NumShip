...
l1 = plt.legend(ld, bbox_to_anchor=(0.0, 1.1, .45, .2), loc=3, ncol=2, mode="expand", borderaxespad=0.)
plt.grid(True)
...
if save:
    plt.savefig(dirzz + TipoModelo +'velo-u-zz.' + formato, format=formato)
    plt.clf()
else:
    plt.show()
    plt.clf()
...
if save:
    plt.savefig(dirzz + TipoModelo +'velo-v-zz.' + formato, format=formato)
    plt.clf()
else:
    plt.show()
    plt.clf()    
...
if save:
    plt.savefig(dirzz + TipoModelo +'velo-r-zz.' + formato, format=formato)
    plt.clf()
else:
    plt.show()
    plt.clf()
