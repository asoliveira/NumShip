...
def qtdf (arq, scgarq, diretorio, qtd=1, derivada = None, multi=None):
  ...
  print 'simulado dados da curva de Zigzag do arquivo'
  print arq
  print 'salvo como ' + arqrel
  ...
  if qtd ==3 or qtd==4:
    ...
    if os.path.exists(bench):
      ...
      arq.split('/')[-1].split('.')[0] + '_bench'):
        if not os.path.exists(scgarq +  '/../../' + 
        arq.split('/')[-1].split('.')[0] + '_bench/zigzag'):        
          shutil.copytree(bench + '/zigzag' ,scgarq + '/../../' + 
          arq.split('/')[-1].split('.')[0] + '_bench/zigzag')
      else:
...
print 'Início da simulação da Curva de Zigzag ...'
if qtd == 1:
    ...
    if 'bench' not in sdir:
      ...
      for sdir2 in ldir2:
        ...
        if os.path.isdir(caminhosdir):
          ...
          if plot:
            execfile('plotp.py')
            print 'Realizada a plotagem do diretório ' + caminhosdir
        os.chdir(os.getcwd().split(scgarq_raiz)[0])
elif qtd == 1 or qtd == 2:
  os.chdir(os.getcwd().split(scgarq_raiz)[0])
