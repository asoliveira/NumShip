#!/usr/bin/env python
# -*- coding: utf-8 -*-

#É adimensional?
adi = False
#É para salvar as figuras(True|False)?
save = True
#Caso seja para salvar, qual é o formato desejado?
formato = 'jpg'
#Caso seja para salvar, qual é o diretório que devo salvar?
dircg = 'fig-sen'
#Caso seja para salvar, qual é o nome do arquivo?
nome = 'cg'
#Qual título colocar no gráficos?
titulo = ''#'Curva de Giro'
#Qual a cor dos gráficos?
pc = 'k'
bc = 'k'
lc = 'k'
brlc = 'k'
rc = 'k'
#Estilo de linha
ps = '-'
bs = '-.'
rs = '.'
ls = '+'
brls = '^'

import os

import scipy as sp
import matplotlib.pyplot as plt

from libplot import *
        
poshis = sp.genfromtxt('../entrada/padrao/CurvaGiro/pos.dat')
poshis2 = sp.genfromtxt('../entrada/beta/saida1.2/CurvaGiro/pos.dat')
poshis3 = sp.genfromtxt('../entrada/r/saida1.2/CurvaGiro/pos.dat')
poshis4 = sp.genfromtxt('../entrada/leme/saida1.2/CurvaGiro/pos.dat')
poshis5 = sp.genfromtxt('../entrada/brl/saida1.2/CurvaGiro/pos.dat')

axl = [-10, 1100, 0, 1000]

#Plotando a Curva de Giro      
if adi:
    ylabel = r'$x\prime$'
    xposlabel = r'$y\prime$'
else:
    ylabel = r'$x \quad m$'    
    xposlabel = r'$y \quad m$'    

plt.subplot2grid((1,4),(0,0), colspan=3)
#Padrao
plt.plot(poshis[:, 2],  poshis[:, 1],  color = pc, linestyle = ps,
linewidth = 1, label=ur'padrão')
plt.plot(poshis2[:, 2],  poshis2[:, 1],  color = bc,linestyle = bs,
linewidth = 2, label=ur'1.2beta')
plt.scatter(somep(poshis3[:, 2], num = 200), somep(poshis3[:, 1], num = 200), 
color = rc, marker = rs, s = 8, label=ur'1.2r')
plt.scatter(somep(poshis4[:, 2], num = 100),  somep(poshis4[:, 1], num = 100), 
color = lc, marker = ls, s = 20, label=ur'1.2leme')
plt.scatter(somep(poshis5[:, 2], num = 100), somep(poshis5[:, 1], num = 100), 
color = brlc, marker = brls, s = 14, label=ur'1.2brl')
plt.title(titulo)
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
plt.ylabel(ylabel)
plt.xlabel(xposlabel)
plt.axis(axl)
plt.grid(True)

if save:
    if not os.path.exists(dircg):
        os.makedirs(dircg)
    if os.path.exists(dircg + '/' + nome + '.' + formato):
        os.remove(dircg + '/' + nome + '.' + formato)
    plt.savefig(dircg + '/' + nome + '.' + formato ,  format=formato)
else:    
    plt.show()           
